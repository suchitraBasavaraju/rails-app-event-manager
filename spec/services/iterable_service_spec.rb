...
RSpec.describe 'IterableService' do
  ...
  describe "send email notification" do
    ...
    it "send notification failed with bad request" do
      WebMock.stub_request(:post, @send_email_url).to_return(status: 400, body: `{ "error" : "Event A creation" }`, headers: {})

      email = "user@example.com"
      response = IterableService.send_email_notification(email)

      expect(response).to eq("Email Notification failed: 400")
    end
    ...
    it "send notification failed with access failure" do
      ...
      expect(response).to eq("Email Notification failed: 401")
    end
  end
  ...
  describe "create event" do
    ...
    it "create Event failed for bad request" do
      ...
      expect(response).to eq("Event A Failed. Error: 400")
    end
    ...
    it "create Event failed for access issue" do
      ...
      expect(response).to eq("Event A Failed. Error: 401")
    end
  end
end
